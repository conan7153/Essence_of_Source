...
/* Introduce new function check_operand() to check that
...
function apply_binary(operator, op1, op2, E) {
    ...
    /*Rewrite the whole conditional expression to statement
    to handle more complex logic. Overloading the > symbol
    with the below logic.
    If op1 is simple function and op2 is list, use function_body
    function to extract the binary operation from the simple
    ...
    */
        ...
        if(is_simple_function(op1) && is_list(op2)){
            const operation = function_body(op1);
            const opr = operator_symbol(operation);
            if(is_binary_operator_combination(function_body(op1))){
                ...
                return map(x => apply_unary(opr, x),
                        op2);
            }
        }else if(is_primitive_function(op1) && is_list(op2)){
            return map(x => apply_primitive_function(op1, list(x)), 
                    op2);
        }else{
            return op1 > op2;
        }
        ...
        return error(operator, "Unknown operator");
    }
}
//
...
function make_application(fun, args) {
    return list("application", fun, args);
}
